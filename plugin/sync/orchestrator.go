...
func (o *Orchestrator) handleCreateOrUpdate(ctx context.Context, event *SyncEvent) error {
	// 确保用户有 Dataset
	datasetID, err := o.EnsureUserDataset(ctx, event.UserID)
	if err != nil {
...
// EnsureUserDataset 确保用户有对应的 RAGFlow Dataset
// 如果不存在则创建新的 Dataset 和 Chat Assistant
func (o *Orchestrator) EnsureUserDataset(ctx context.Context, userID int32) (string, error) {
	...
	dataset, err := o.ragflowClient.CreateDataset(ctx, &ragflow.CreateDatasetRequest{
		...
		ChunkMethod: ragflow.ChunkMethodNaive, // 使用默认分块方法
	})
	if err != nil {
		return "", errors.Wrap(err, "创建 RAGFlow Dataset 失败")
	}
...
// GetUserDatasetID 获取用户的 Dataset ID（不自动创建）
...
func (o *Orchestrator) RunBatchSync(ctx context.Context) error {
	...
	// 1. 同步待处理的 Memo
	memoSuccess, memoFail, err := o.memoSyncer.SyncPendingMemos(ctx, func(ownerID int32) (string, error) {
		return o.EnsureUserDataset(ctx, ownerID)
	}, o.batchSize)
	if err != nil {
	...
	// 2. 同步待处理的附件
	attachmentSuccess, attachmentFail, err := o.attachmentSyncer.SyncPendingAttachments(ctx, func(ownerID int32) (string, error) {
		return o.EnsureUserDataset(ctx, ownerID)
	}, o.batchSize)
	if err != nil {
...
func (o *Orchestrator) retryFailedStates(ctx context.Context) (int, int) {
	...
	for _, state := range retryableStates {
		datasetID, err := o.EnsureUserDataset(ctx, state.OwnerID)
		if err != nil {
...
func (o *Orchestrator) HandleUserDeletion(ctx context.Context, userID int32) error {
	...
	if mapping.DatasetID != "" {
		if err := o.ragflowClient.DeleteDataset(ctx, mapping.DatasetID); err != nil {
	...
	if mapping.AssistantID != "" {
		if err := o.ragflowClient.DeleteChatAssistant(ctx, mapping.AssistantID); err != nil {
