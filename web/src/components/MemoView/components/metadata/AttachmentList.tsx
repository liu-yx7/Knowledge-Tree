import { FileIcon, PaperclipIcon } from "lucide-react";
import { useMemo, useState } from "react";
import { InlineAttachmentPreview } from "@/components/attachment/InlineAttachmentPreview";
import type { Attachment } from "@/types/proto/api/v1/attachment_service_pb";
import { getAttachmentType } from "@/utils/attachment";
import { formatFileSize, getFileTypeLabel } from "@/utils/format";
import AttachmentCard from "./AttachmentCard";
import SectionHeader from "./SectionHeader";

interface AttachmentListProps {
  attachments: Attachment[];
}

// Type guards for attachment types
const isImageAttachment = (attachment: Attachment): boolean => getAttachmentType(attachment) === "image/*";
const isVideoAttachment = (attachment: Attachment): boolean => getAttachmentType(attachment) === "video/*";
const isMediaAttachment = (attachment: Attachment): boolean => isImageAttachment(attachment) || isVideoAttachment(attachment);

// Separate attachments into media (images/videos) and documents
const separateMediaAndDocs = (attachments: Attachment[]): { media: Attachment[]; docs: Attachment[] } => {
  const media: Attachment[] = [];
  const docs: Attachment[] = [];

  for (const attachment of attachments) {
    if (isMediaAttachment(attachment)) {
      media.push(attachment);
    } else {
      docs.push(attachment);
    }
  }

  return { media, docs };
};

const DocumentItem = ({
  attachment,
  isSelected,
  onClick,
}: {
  attachment: Attachment;
  isSelected: boolean;
  onClick: () => void;
}) => {
  const fileTypeLabel = getFileTypeLabel(attachment.type);
  const fileSizeLabel = attachment.size ? formatFileSize(Number(attachment.size)) : undefined;

  return (
    <div
      onClick={onClick}
      className={`flex items-center gap-1 px-1.5 py-1 rounded transition-colors whitespace-nowrap cursor-pointer ${
        isSelected ? "bg-primary/10 ring-1 ring-primary/30" : "hover:bg-accent/20"
      }`}
    >
      <div className="shrink-0 w-5 h-5 rounded overflow-hidden bg-muted/40 flex items-center justify-center">
        <FileIcon className="w-3 h-3 text-muted-foreground" />
      </div>
      <div className="flex items-center gap-1 min-w-0">
        <span className="text-xs truncate" title={attachment.filename}>
          {attachment.filename}
        </span>
        <div className="flex items-center gap-1 text-xs text-muted-foreground shrink-0">
          <span className="text-muted-foreground/50">•</span>
          <span>{fileTypeLabel}</span>
          {fileSizeLabel && (
            <>
              <span className="text-muted-foreground/50">•</span>
              <span>{fileSizeLabel}</span>
            </>
          )}
        </div>
      </div>
    </div>
  );
};

const MediaGrid = ({
  attachments,
  selectedAttachment,
  onSelect,
}: {
  attachments: Attachment[];
  selectedAttachment: Attachment | null;
  onSelect: (attachment: Attachment) => void;
}) => {
  return (
    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2">
      {attachments.map((attachment) => {
        const isSelected = selectedAttachment?.name === attachment.name;
        return (
          <div
            key={attachment.name}
            className={`aspect-square rounded-lg overflow-hidden bg-muted/40 border transition-all cursor-pointer group ${
              isSelected ? "border-primary ring-1 ring-primary/30" : "border-border hover:border-accent/50"
            }`}
            onClick={() => onSelect(attachment)}
          >
            <div className="w-full h-full relative">
              <AttachmentCard attachment={attachment} className="rounded-none" />
              {getAttachmentType(attachment) === "video/*" && (
                <div className="absolute inset-0 flex items-center justify-center bg-black/30 group-hover:bg-black/40 transition-colors">
                  <div className="w-8 h-8 rounded-full bg-white/80 flex items-center justify-center">
                    <svg className="w-5 h-5 text-black fill-current ml-0.5" viewBox="0 0 24 24">
                      <path d="M8 5v14l11-7z" />
                    </svg>
                  </div>
                </div>
              )}
            </div>
          </div>
        );
      })}
    </div>
  );
};

const DocsList = ({
  attachments,
  selectedAttachment,
  onSelect,
}: {
  attachments: Attachment[];
  selectedAttachment: Attachment | null;
  onSelect: (attachment: Attachment) => void;
}) => (
  <div className="flex flex-col gap-0.5">
    {attachments.map((attachment) => (
      <DocumentItem
        key={attachment.name}
        attachment={attachment}
        isSelected={selectedAttachment?.name === attachment.name}
        onClick={() => onSelect(attachment)}
      />
    ))}
  </div>
);

const AttachmentList = ({ attachments }: AttachmentListProps) => {
  const [selectedAttachment, setSelectedAttachment] = useState<Attachment | null>(null);
  const { media: mediaItems, docs: docItems } = useMemo(() => separateMediaAndDocs(attachments), [attachments]);

  if (attachments.length === 0) {
    return null;
  }

  const handleSelect = (attachment: Attachment) => {
    // Toggle selection - click again to close
    if (selectedAttachment?.name === attachment.name) {
      setSelectedAttachment(null);
    } else {
      setSelectedAttachment(attachment);
    }
  };

  const handleClose = () => {
    setSelectedAttachment(null);
  };

  const handleNavigate = (attachment: Attachment) => {
    setSelectedAttachment(attachment);
  };

  return (
    <div className="w-full">
      <div className="w-full rounded-lg border border-border bg-muted/20 overflow-hidden">
        <SectionHeader icon={PaperclipIcon} title="Attachments" count={attachments.length} />

        <div className="p-2 flex flex-col gap-1">
          {mediaItems.length > 0 && (
            <MediaGrid attachments={mediaItems} selectedAttachment={selectedAttachment} onSelect={handleSelect} />
          )}

          {mediaItems.length > 0 && docItems.length > 0 && <div className="border-t mt-1 border-border opacity-60" />}

          {docItems.length > 0 && (
            <DocsList attachments={docItems} selectedAttachment={selectedAttachment} onSelect={handleSelect} />
          )}
        </div>
      </div>

      {/* Inline preview - shown below the attachment list */}
      {selectedAttachment && (
        <InlineAttachmentPreview
          attachment={selectedAttachment}
          attachments={attachments}
          onClose={handleClose}
          onNavigate={handleNavigate}
        />
      )}
    </div>
  );
};

export default AttachmentList;
