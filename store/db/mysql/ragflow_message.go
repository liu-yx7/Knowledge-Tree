...
func (d *DB) CreateRAGFlowMessage(ctx context.Context, create *store.RAGFlowMessage) (*store.RAGFlowMessage, error) {
	...
	stmt := "INSERT INTO `ragflow_message` (" + strings.Join(fields, ", ") + ") VALUES (" + strings.Join(placeholder, ", ") + ")"
	...
	if err := d.db.QueryRowContext(ctx, "SELECT `created_ts` FROM `ragflow_message` WHERE `id` = ?", create.ID).Scan(&create.CreatedTs); err != nil {
		return nil, err
	}
